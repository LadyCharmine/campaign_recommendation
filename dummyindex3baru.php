...
function apriori($conn, $min_support, $min_confidence, $filtered_campaign_ids = []) {
    ...
    foreach ($rules as $rule) {
        ...
        $query = empty($filtered_campaign_ids) 
            ? "SELECT id, judul, deskripsi, yayasan, kategori, lokasi, sisa_hari, gambar, terkumpul FROM campaigns WHERE kategori LIKE ?"
            : "SELECT id, judul, deskripsi, yayasan, kategori, lokasi, sisa_hari, gambar, terkumpul FROM campaigns WHERE kategori LIKE ? AND id IN (" . implode(',', array_map('intval', $filtered_campaign_ids)) . ")";
...
if (isset($_SESSION['user_id'])) {
    ...
    if ($is_search_active) {
        // Saat pencarian aktif, hanya gunakan cb_recommendations dan apriori_recommendations
        $campaigns = array_merge($cb_recommendations, $apriori_recommendations);
    } else {
        // Saat pencarian tidak aktif, gunakan semua rekomendasi
        ...
        if (!empty($donated_campaign_ids)) {
            ...
            $query = "SELECT id, judul, deskripsi, yayasan, kategori, lokasi, sisa_hari, gambar, terkumpul FROM campaigns WHERE id NOT IN ($in_clause) AND (kategori IN (SELECT kategori FROM campaigns WHERE id IN ($in_clause)) OR lokasi IN (SELECT lokasi FROM campaigns WHERE id IN ($in_clause)))";
            $result = $conn->query($query);
            while ($row = $result->fetch_assoc()) {
                $expert_rating = 0;
                try {
        ...
        // Gabungkan semua rekomendasi saat pencarian tidak aktif
    ...
    // Jika tidak ada user_id, tampilkan semua kampanye saat pencarian tidak aktif
    $campaigns = $is_search_active ? array_merge($cb_recommendations, $apriori_recommendations) : $all_campaigns;
}
        ...
        <div class="container mx-auto px-6 py-32 text-center">
            ...
            <div id="searchForm" class="mt-8 max-w-3xl mx-auto bg-white bg-opacity-90 rounded-lg p-6 shadow-lg hidden">
                ...
                <form id="searchFormElement" method="GET" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-gray-700 text-sm font-medium">Jenis</label>
