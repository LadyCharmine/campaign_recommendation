...
function apriori($conn, $min_support, $min_confidence, $filtered_campaign_ids = []) {
    ...
    foreach ($rules as $rule) {
        ...
        $query = empty($filtered_campaign_ids) 
            ? "SELECT id, judul, deskripsi, yayasan, kategori, lokasi, sisahari, gambar, terkumpul FROM campaigns WHERE kategori LIKE ?"
            : "SELECT id, judul, deskripsi, yayasan, kategori, lokasi, sisahari, gambar, terkumpul FROM campaigns WHERE kategori LIKE ? AND id IN (" . implode(',', array_map('intval', $filtered_campaign_ids)) . ")";
...
$all_campaigns = [];
$result = $conn->query("SELECT id, judul, deskripsi, yayasan, kategori, lokasi, sisahari, gambar, terkumpul FROM campaigns");
while ($row = $result->fetch_assoc()) {
    $expert_rating = 0;
    try {
        $stmt = $conn->prepare("SELECT AVG(rating) as avg_rating FROM expert_ratings WHERE campaign_id = ?");
        if ($stmt) {
            $stmt->bind_param("i", $row['id']);
            $stmt->execute();
            $rating_result = $stmt->get_result()->fetch_assoc();
            $expert_rating = $rating_result['avg_rating'] ? $rating_result['avg_rating'] / 5 : 0;
            $stmt->close();
        }
...
// Pencarian content-based
$cb_recommendations = [];
$is_search_active = ($search_keyword || $search_category || $search_location || $search_deadline > 0 || $search_budget);
if ($is_search_active) {
    $query_text = ($search_keyword ? $search_keyword . " " : "") . ($search_category ? $search_category . " " : "") . ($search_location ? $search_location : "");
    if (!empty(trim($query_text)) || $search_deadline > 0) {
        ...
        foreach ($all_campaigns as $campaign_id => $campaign) {
            if ($search_deadline > 0 && $campaign['sisahari'] !== '∞' && (int)$campaign['sisahari'] > $search_deadline) {
                continue;
            }
...
// Apriori berdasarkan filtered campaigns
...
if (isset($_SESSION['user_id'])) {
    ...
    if (!empty($donated_campaign_ids)) {
        ...
        $query = "SELECT id, judul, deskripsi, yayasan, kategori, lokasi, sisahari, gambar, terkumpul FROM campaigns WHERE id NOT IN ($in_clause) AND (kategori IN (SELECT kategori FROM campaigns WHERE id IN ($in_clause)) OR lokasi IN (SELECT lokasi FROM campaigns WHERE id IN ($in_clause)))";
        $result = $conn->query($query);
        while ($row = $result->fetch_assoc()) {
            if ($search_deadline > 0 && $row['sisahari'] !== '∞' && (int)$row['sisahari'] > $search_deadline) {
                continue;
            }
            $expert_rating = 0;
            try {
    ...
    foreach ($user_categories as $user_cat) {
        $query = "SELECT id, judul, deskripsi, yayasan, kategori, lokasi, sisahari, gambar, terkumpul FROM campaigns WHERE kategori LIKE ?";
        $stmt = $conn->prepare($query);
        $like_param = "%$user_cat%";
        $stmt->bind_param("s", $like_param);
        $stmt->execute();
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $expert_rating = 0;
            try {
                $stmt_rating = $conn->prepare("SELECT AVG(rating) as avg_rating FROM expert_ratings WHERE campaign_id = ?");
                if ($stmt_rating) {
                    $stmt_rating->bind_param("i", $row['id']);
                    $stmt_rating->execute();
    ...
    if ($search_budget && isset($user_budget[$search_budget]) && $user_budget[$search_budget] > 0) {
        foreach ($all_campaigns as $campaign_id => $campaign) {
            $days_remaining = $campaign['sisahari'] === '∞' ? 30 : (int)$campaign['sisahari'];
            $days_remaining = max($days_remaining, 1); // Hindari pembagian dengan nol
            $avg_donation_per_day = $campaign['terkumpul'] / $days_remaining;

            $budget_value = 0;
            if ($search_budget === 'daily') {
                $budget_value = $avg_donation_per_day;
            } elseif ($search_budget === 'weekly') {
                $budget_value = $avg_donation_per_day * 7;
            } elseif ($search_budget === 'monthly') {
                $budget_value = $avg_donation_per_day * 30;
            }

            if ($budget_value <= $user_budget[$search_budget]) {
                $budget_filtered_recommendations[$campaign_id] = $campaign;
            }
        }
    }

    $campaigns = array_merge($cb_recommendations, $apriori_recommendations, $donation_based_recommendations, $preference_based_recommendations, $budget_filtered_recommendations, $all_campaigns);
} else {
...
foreach ($campaigns as $campaign) {
    if (!isset($unique_campaigns[$campaign['id']])) {
        ...
        if ($campaign['expert_rating'] > $unique_campaigns[$campaign['id']]['expert_rating']) {
...
foreach ($campaigns as &$cargo) {
    $similarity = $cargo['similarity'] ?? 0;
    $confidence = ($cargo['confidence'] ?? 0) / 100;
    $preference_boost = $cargo['preference_boost'] ?? 0;
    $expert_rating = $cargo['expert_rating'] ?? 0;
    $cargo['combined_score'] = ($similarity * 0.4) + ($confidence * 0.2) + ($preference_boost * 0.2) + ($expert_rating * 0.2);
}
unset($cargo);
...
if (isset($_GET['select']) && isset($_SESSION['user_id'])) {
    ...
    $stmt = $conn->prepare("SELECT judul, deskripsi, kategori, lokasi, sisahari, terkumpul FROM campaigns WHERE id = ?");
        ...
        <div class="container mx-auto px-6 py-32 text-center">
            ...
            <div id="searchForm" class="mt-8 max-w-3xl mx-auto bg-white bg-opacity-90 rounded-lg p-6 shadow-lg hidden">
                ...
                <form id="searchFormElement" method="GET" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-gray-700 text-sm font-medium">Jenis Anggaran</label>
                        <select name="budget" class="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600">
                            <option value="">Pilih Jenis</option>
                            <option value="daily" <?php echo ($search_budget == 'daily') ? 'selected' : ''; ?>>Harian</option>
                            <option value="weekly" <?php echo ($search_budget == 'weekly') ? 'selected' : ''; ?>>Mingguan</option>
                            <option value="monthly" <?php echo ($search_budget == 'monthly') ? 'selected' : ''; ?>>Bulanan</option>
                        </select>
                    </div>
                ...
                <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <p class="text-sm text-gray-500"><strong>Sisa Hari:</strong> <?php echo htmlspecialchars($row['sisahari']); ?></p>
                            <p class="text-sm text-gray-500"><strong>Terkumpul:</strong> Rp <?php echo number_format($row['terkumpul'], 0, ',', '.'); ?></p>
