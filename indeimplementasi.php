...
function apriori($conn, $min_support, $min_confidence, $filtered_campaign_ids = []) {
    ...
    // Kueri berdasarkan filtered_campaign_ids jika ada
    ...
    if (!$result) {
        error_log("Error: Gagal mengambil data kategori dari campaigns: " . $conn->error);
        return [];
    }

    while ($row = $result->fetch_assoc()) {
        $categories = array_map('trim', explode(',', $row['kategori']));
        if (!empty($categories) && count($categories) > 1) { // Pastikan ada lebih dari 1 kategori
            $transactions[] = $categories;
        }
    }
    $total_transactions = count($transactions);
    if ($total_transactions == 0) {
        error_log("Error: Tidak ada transaksi kategori yang ditemukan.");
        return [];
    }
    ...
    foreach ($rules as $rule) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if (in_array($consequent, $categories)) {
                $expert_rating = 0;
                try {
                    $rating_stmt = $conn->prepare("SELECT AVG(rating) as avg_rating FROM expert_ratings WHERE campaign_id = ?");
                    if ($rating_stmt) {
                    ...
                    error_log("Error fetching expert rating: " . $e->getMessage());
                    $expert_rating = 0;
                }
