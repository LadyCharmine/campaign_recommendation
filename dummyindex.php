...
// Fungsi Apriori
$min_support = 0.1; // Turunkan ke 10%
$min_confidence = 0.3; // Turunkan ke 30%

function apriori($conn, $min_support, $min_confidence, $filtered_campaign_ids = []) {
    $transactions = [];
    
    // Jika ada filtered_campaign_ids, hanya ambil kampanye yang sesuai
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (!empty($categories) && count($categories) > 1) { // Pastikan lebih dari 1 kategori
...
$cb_recommendations = [];
if ($search_category || $search_location || $search_deadline > 0) {
...
if (isset($_SESSION['user_id'])) {
    ...
    // Untuk guest, gunakan semua kampanye dari database tanpa filter rekomendasi
    $campaigns = $all_campaigns;
    if (!empty($cb_recommendations)) {
        $campaigns = $cb_recommendations; // Jika ada pencarian, prioritaskan hasil pencarian
    }
}
                ...
                <div class="bg-white rounded-xl shadow-lg overflow-hidden transform transition hover:scale-105">
                    ...
                    <div class="p-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <?php if ($search_category || $search_deadline > 0 || $search_location): ?>
